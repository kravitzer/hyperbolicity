...
int main()
{
	try
	{
		...
		clock_t t1 = clock();
		graph_ptr_t graph = GraphAlgorithms::LoadGraphFromFile("C:\\Users\\Eran\\Dropbox\\University\\Thesis\\Shavitt\\Code\\Graphs\\Stanford\\Formatted\\Email-Enron.txt"); //C:\\Users\\Eran\\Dropbox\\University\\Thesis\\Shavitt\\Code\\Graphs\\Stanford\\Formatted\\roadNet-PA.txt");
		...
		if (nodeCount != graph->size()) cout << "***ERROR*** - Node count is different than the sum of the sccs!" << endl;
		if (edgeCount != (graph->edgeCount() / 2) ) cout << "***ERROR*** - Edge count is different than the sum of the sccs!" << endl;
